...
- [x] **Chrome Web Store 출시** — [kcgedhigobhicnaedgkmojhiceacmjmo](https://chromewebstore.google.com/detail/notebooklm-podcast-sync/kcgedhigobhicnaedgkmojhiceacmjmo) (v0.4.30+)
- [x] **v0.4.42 버그 수정 및 최적화** — ① empty notebook 3초 대기 버그 수정 (content.js ready 조건), ② popup awaitPushResult 타임아웃 15분으로 정렬, ③ skippedShortIds sync quota 무조건 20% 컷, ④ setTaskState storage.session 500ms debounce (고빈도 progress beacon), ⑤ notifyBulkComplete async i18n (uiLang 읽어 한·영·독 알림), ⑥ ghList 30초 TTL 인메모리 캐시 + push 성공 시 즉시 무효화, ⑦ FILENAME_RE feed.js 단일 export (중복 제거, `i` 플래그 추가), ⑧ AudioContext offscreen document 수명 동안 재사용 (매 transcode 마다 new/close 제거)
...
[Chrome 웹 스토어](https://chromewebstore.google.com/detail/notebooklm-podcast-sync/kcgedhigobhicnaedgkmojhiceacmjmo)에서 바로 설치할 수 있습니다.
